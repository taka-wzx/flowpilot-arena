...
def test_actor_resolution_and_every_tenant_query_are_organization_qualified(
    ...
    with Session(database_engine) as session:
        ...
        assert len(list_users(session, alpha, alpha.organization_id)) == 3
        assert count_users(session, alpha, alpha.organization_id) == 3
        assert count_memberships(session, alpha, alpha.organization_id) == 3
