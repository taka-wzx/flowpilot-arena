...
export function IamPage() {
  ...
  async function revoke(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(form);
    try {
      await updateRecord<Account>(
        `/api/iam/employees/${Number(data.get("employee_id"))}/revoke`,
        {},
      );
      form.reset();
      await refresh();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Unable to revoke account");
    }
  }
  ...
  return (
    <section>
      ...
      {showTransitions && <form onSubmit={revoke}>
        <label>Revoke account employee ID<input name="employee_id" type="number" min="1" required /></label>
