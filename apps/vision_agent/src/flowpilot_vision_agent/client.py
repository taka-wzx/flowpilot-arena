...
class BrowserWorkerClient:
    ...
    async def create_session(self) -> VisionSessionCreated:
        response = await self._client.post(
            f"{self._base_url}/api/browser/vision-sessions",
            json={"schema_version": "w5-vision-session/1.0", "initial_path": "/hris"},
        )
        response.raise_for_status()
        return VisionSessionCreated.model_validate_json(response.text)

    async def execute_action(
        self, session_id: str, action: VisionBrowserAction
    ) -> VisionActionResult:
        response = await self._client.post(
            f"{self._base_url}/api/browser/vision-sessions/{session_id}/actions",
            json=action.model_dump(mode="json"),
        )
        response.raise_for_status()
        return VisionActionResult.model_validate_json(response.text)

    async def close_session(self, session_id: str) -> None:
        response = await self._client.delete(
            f"{self._base_url}/api/browser/vision-sessions/{session_id}"
        )
        response.raise_for_status()
