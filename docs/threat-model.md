# W11 threat model - HITL, risk, one-time approval, and audit chain

## Scope and assets

W11 preserves the fixed local OIDC trust roots, bearer/JWKS verification,
tenant isolation, and optimistic locking while additionally protecting the
closed risk mapping and strict parameter bindings; manager/security authority;
approval request/decision/grant state; raw one-time credential boundary;
durable execution/recovery references; and organization-local audit chains.
It continues to protect
...
| Threat | W11 control | Remaining limitation |
|---|---|---|
...
| Cross-organization get/list/count | SQL is organization-qualified before object/count/version lookup; uniform 404 | Database administrator compromise is outside W11 |
...
| Last-write-wins loses an update | Required strong If-Match and atomic organization/resource/expected-version SQL | Multi-region conflict resolution is outside W11 |
...
W12 production API/Worker split, scheduling, rate limiting, backpressure, load
and `v0.3.0`; W13 telemetry; W14 malicious-page and full
...
glass operation remain outside W11.
